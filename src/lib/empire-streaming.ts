...
async function getSlug(
  ...
  const response = await fetch(
    'https://empire-streaming.pro/api/travelguard/prev',
    {
      method: 'POST',
...
async function getData(content: number, type: string): Promise<ContentData> {
  const url = `https://empire-streaming.pro/api/${
...
async function getEpisodeSerie(
  ...
  const response: ContentVideos = await fetch(
    `https://empire-streaming.pro/api/series/${idContent}`
  ).then((res) => res.json());
...
async function getFilm(
  ...
  const response: ContentVideos = await fetch(
    `https://empire-streaming.pro/api/films_anonymous/${idContent}`
  ).then((res) => res.json());
...
async function getVideo(
  ...
  const response = await fetch(
    'https://empire-streaming.pro/api/travelguard/get_data',
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json;charset=utf-8' },
      body: JSON.stringify({
